...
def evaluate_and_plot(config: Config):
    ...
    if not checkpoint_path.exists():
        ...
        raise FileNotFoundError(f"Bitte trainiere zuerst das Modell, um {checkpoint_path.name} zu erzeugen.")
    ...
    plt.plot(preds, label="Predicted CPU (GRU Baseline)", color="crimson", linestyle="--", alpha=0.9, linewidth=2)
    
    plt.title("Infrastructure Forecast Dashboard: Actual vs. Predicted CPU", fontsize=14, fontweight="bold")
