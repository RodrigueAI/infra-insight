...
def train_model():
    ...
    df = pd.read_csv("../data/external/aws_cpu_utilization.csv", parse_dates=["timestamp"], index_col="timestamp")
    ...
    train_dataset, test_dataset, _ = prepare_data(df_enriched, target_col="value", seq_length=SEQ_LENGTH)
    
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=False) # Keine Shuffles bei Zeitreihen!
    test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False)

    # 3. Modell instanziieren
    input_dim = df_enriched.shape[1] - 1 # Alle Spalten außer 'value'
    model = CPUForecastGRU(input_dim=input_dim, hidden_dim=HIDDEN_DIM, output_dim=1, num_layers=NUM_LAYERS).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"Epoch [{epoch}/{EPOCHS}] -> Train Loss (MSE): {train_loss:.4f} | Test Loss (MSE): {test_loss:.4f}")
