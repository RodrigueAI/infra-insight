...
def run_spark_ingestion(config: Config):
    ...
    hub_server = df_vault.select("hk_server", "instance_id", "load_timestamp", "record_source").distinct()
    sat_telemetry = df_vault.select(
        "hk_server", "timestamp", "load_timestamp", "hash_diff", 
        F.col("value").alias("cpu_utilization"), "record_source"
    )
