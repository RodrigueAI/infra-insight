...
def run_spark_ingestion(config: Config):
    ...
    df_vault = (df_raw
                .withColumn("instance_id", F.lit(SRC_INSTANCE_ID))
    ...
    df_vault.select("hk_server", "instance_id", "load_timestamp", "record_source").distinct().write.mode("overwrite").parquet(str(hub_path))
    df_vault.select("hk_server", "timestamp", "load_timestamp", "hash_diff", F.col("value").alias("cpu_utilization"), "record_source").write.mode("overwrite").parquet(str(sat_path))
...
def run_spark_feature_engineering():
    ...
    logger.info("Berechne zyklische Zeit-Features (Stunde & Wochentag) und rollierende Statistiken...")
    ...
    logger.info(f"✅ Multivariate Features inkl. Wochentag erfolgreich unter '{output_path}' persistiert.")
